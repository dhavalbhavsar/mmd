...
class ApiController extends AbstractController
{
     ...
     * @return JsonResponse
     */
    public function respondWithSuccess(string $success, $data = [], $headers = []): JsonResponse
    {
        $data = [
            'status' => $this->getStatusCode(),
            'success' => $success,
     ...
     * @return JsonResponse
     */
    public function respondValidationError($validation = [], string $message = 'Validation Error!', $status = 422): JsonResponse
    {
        $data = [
            'status' => $status,
            'message' => $message,
        ...
        if($validation instanceof ConstraintViolationListInterface) {
            ...
            foreach ($validation as $error) {
                $error_response[$error->getPropertyPath()] = $error->getMessage();
            }
            return new JsonResponse(['errors' => $error_response], $status);
        }
